...
import { useEventProperties, useSettingProperties, usePortal } from '../utils';
...
export const useInfoWindow = (props = {} as UseInfoWindow) => {
  ...
  useLayoutEffect(() => {
    ...
    if (!infoWindow) {
      ...
      let instance: AMap.InfoWindow = new AMap.InfoWindow({
        ...other,
  ...
  useEffect(() => () => map && infoWindow && map.remove(infoWindow), []);
